...
# QKD key generation between Alice and Bob
def perform_qkd_with_bob(connection):
    n_bits = 100
    alice_bits = np.random.randint(2, size=n_bits)
    alice_bases = np.random.randint(2, size=n_bits)

    # Convert Alice's bases and bits to strings
    alice_bits_str = ''.join(map(str, alice_bits))
    alice_bases_str = ''.join(map(str, alice_bases))

    # Debug: Print Alice's bases and bits
    # print("Alice's bases: ", alice_bases_str)
    # print("Alice's bits: ", alice_bits_str)

    # Send Alice's bases to Bob as a string
    connection.sendall(alice_bases_str.encode())

    # Receive Bob's bases as a string
    bob_bases_data = connection.recv(1024)
    bob_bases_str = bob_bases_data.decode()
    bob_bases = np.array([int(x) for x in bob_bases_str])

    # Debug: Print Bob's received bases
    #print("Bob's bases: ", bob_bases)

    # Send Alice's bits to Bob as a string
    connection.sendall(alice_bits_str.encode())

    # Generate qubits based on Alice's bits and bases
    alice_qubits = []
...
def receive_message():
    ...
    while not stop_event.is_set():  # Run until stop_event is set
        ...
        try:
            ...
            shared_key = perform_qkd_with_bob(conn)
...
def send_message(peer_ip, peer_port, message):
    ...
    # Perform QKD to generate shared AES key
    shared_key = perform_qkd_with_bob(sender_socket)
    aes_key = get_aes_key(shared_key)
