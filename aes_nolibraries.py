# AES S-box and RCON constants
SBOX = [
    
]


RCON = [
    # Add values for RCON required by AES (first 10 rounds for AES-128)
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
]

def xor_bytes(a, b):
    return bytes(i ^ j for i, j in zip(a, b))

def sub_bytes(word):
    return bytes(SBOX[b] for b in word)

def rotate(word):
    return word[1:] + word[:1]

# Key expansion function for AES-128
def key_expansion(key):
    key_schedule = [key[i:i+4] for i in range(0, len(key), 4)]
    for i in range(4, 4 * 11):  # Generate enough keys for 10 rounds + initial round
        temp = key_schedule[i - 1]
        if i % 4 == 0:
            temp = xor_bytes(sub_bytes(rotate(temp)), bytes([RCON[i // 4 - 1], 0, 0, 0]))
        key_schedule.append(xor_bytes(key_schedule[i - 4], temp))
    return key_schedule

# AES round functions
def add_round_key(state, key):
    return xor_bytes(state, key)

def sub_bytes_state(state):
    return bytes(SBOX[b] for b in state)

def shift_rows(state):
    return bytes([state[i % 4 * 4 + (i + i // 4) % 4] for i in range(16)])

def mix_columns(state):
    def xtime(a):
        return ((a << 1) ^ 0x1B) & 0xFF if a & 0x80 else a << 1

    def mix_column(column):
        t = column[0] ^ column[1] ^ column[2] ^ column[3]
        return [
            column[0] ^ t ^ xtime(column[0] ^ column[1]),
            column[1] ^ t ^ xtime(column[1] ^ column[2]),
            column[2] ^ t ^ xtime(column[2] ^ column[3]),
            column[3] ^ t ^ xtime(column[3] ^ column[0])
        ]
    
    state = bytearray(state)
    for i in range(0, 16, 4):
        column = mix_column(state[i:i+4])
        state[i:i+4] = column
    return bytes(state)

# AES encryption for one block
def aes_encrypt_block(plaintext, key_schedule):
    state = add_round_key(plaintext, key_schedule[0])
    for round in range(1, 10):
        state = sub_bytes_state(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, key_schedule[round])
    state = sub_bytes_state(state)
    state = shift_rows(state)
    state = add_round_key(state, key_schedule[10])
    return state

# CBC mode encryption
def aes_encrypt_cbc(plaintext, key, iv):
    key_schedule = key_expansion(key)
    ciphertext = bytearray()
    previous_block = iv
    for i in range(0, len(plaintext), 16):
        block = plaintext[i:i+16]
        if len(block) < 16:
            block += bytes(16 - len(block))  # Padding with zeros if needed
        encrypted_block = aes_encrypt_block(xor_bytes(block, previous_block), key_schedule)
        ciphertext.extend(encrypted_block)
        previous_block = encrypted_block
    return bytes(ciphertext)

# Test parameters
plaintext = b"Hello AES Encryption!"
key = b"16bytekeyforAES!!"  # 128-bit key
iv = b"16byteinitvector!"  # 128-bit IV

# Encrypt the plaintext
ciphertext = aes_encrypt_cbc(plaintext, key, iv)
print("Ciphertext:", ciphertext)
